...
describe('Device module', () => {
  ...
  describe('detects device', () => {
    it('Samsung Galaxy S9', () => {
      ...
      expect(extractDevices(ctx, userAgent)).toEqual({ isTV: false, android: true, ios: false, macOS: false, windows: false })
    })

    it('Apple iPhone X', () => {
      const userAgent = 'Mozilla/5.0 (iPhone; CPU iPhone OS 11_0 like Mac OS X) AppleWebKit/604.1.38 (KHTML, like Gecko) Version/11.0 Mobile/15A372 Safari/604.1'

      expect(extractDevices(ctx, userAgent)).toEqual({isTV: false, android: false, ios: true, macOS: true, windows: false })
    })
    ...
    it('Windows 10-based PC using Edge browser', () => {
      ...
      expect(extractDevices(ctx, userAgent)).toEqual({ isTV: false, android: false, ios: false, macOS: false, windows: true })
    })
    ...
    it('Mac OS X-based computer using a Safari browser', () => {
      ...
      expect(extractDevices(ctx, userAgent)).toEqual({ isTV: false, android: false, ios: false, macOS: true, windows: false })
    })
  })
})
