...
function extractDevices (ctx, userAgent = DEFAULT_USER_AGENT) {
  ...
  return { mobile, mobileOrTablet, tv, ios, android, windows, macOS }
}
...
export default async function (ctx, inject) {
  ...
  const { mobile, mobileOrTablet, tv, ios, android, windows, macOS } = extractDevices(ctx, userAgent)

  ctx.isMobile = mobile
  ctx.isMobileOrTablet = mobileOrTablet
  ctx.isTV = tv
  ctx.isTablet = !mobile && mobileOrTablet
  ctx.isDesktop = !mobileOrTablet && !tv
  ctx.isDesktopOrTablet = !mobile && !tv
  ctx.isIos = ios
  ...
  inject('device', {
    ...
    isDesktop: !mobileOrTablet && !tv,
    isIos: ios,
    isAndroid: android,
    isWindows: windows,
    isMacOS: macOS,
    isDesktopOrTablet: !mobile && !tv
  })
}
